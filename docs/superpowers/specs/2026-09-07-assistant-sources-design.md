...
## 데이터 모델 (마이그레이션 0020 — 0019은 첨부 파일용으로 이미 쓰였다)
...
`lib/categories.ts`의 `Game.sheetId`는 없앤다. 0018이 anon에 준 열 단위 `select` 권한은 `sheet_id`를 포함하지 않으므로 그대로 둔다.
...
0020 이전에 저장된 `pending` 제안은 `sourceId`가 없다. 적용 라우트는 이를 `invalid_proposal`로 실패시킨다. 개발 중 데이터뿐이라 별도 변환은 하지 않는다.
...
- CLAUDE.md 8번 항목과 "운영 시트 어시스턴트 설정 절차"를 갱신: 시트·문서 여러 개, Cloud 콘솔에서 "Google Docs API"도 사용 설정, 0020 실행.
