...
- 브랜치: `feat/sheet-assistant` 위에서 시작한다(`feat/openai-suggest`). 어시스턴트 마이그레이션 0015~0017은 main의 0015(번역)·0016(자동답변 지연, 미커밋)과 번호가 겹치므로 **0017~0019로 옮기고**, 임베딩은 **0020**이다.
...
## 데이터 모델 (마이그레이션 0020 `0020_inquiry_embeddings.sql`)
...
- `CLAUDE.md`: 핵심 기능에 "AI 답변 추천" 항목을 추가해 OpenAI·유사 답변·자료 근거·백필 절차를 적고, 어시스턴트 항목의 마이그레이션 번호를 고친다. 설정 절차 절에 "AI 답변 추천 설정 절차"(0020 실행, 백필 스크립트 실행)를 더한다.
