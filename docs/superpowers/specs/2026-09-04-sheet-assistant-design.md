...
- 허용: txt, md, csv, tsv, json, xlsx. 파일당 2MB, 메시지당 5개, 대화당 첨부 텍스트 합계 200,000자(`lib/attachment-rules.ts`). 화면은 형식·크기를 고르는 즉시 거르고, 서버가 다시 검사해 `unsupported_type` / `file_too_large` / `too_many_files` / `attachments_too_large` / `file_unreadable`로 400을 돌려준다. 합계 초과는 저장 전에 거절하므로 파일을 빼고 다시 보낼 수 있다.
...
- 저장: `assistant_messages.attachments jsonb` = `[{ name, size, text }]`(마이그레이션 0016). 화면에는 이름·크기만 내려간다.
...
3. 마이그레이션 `0015_assistant.sql` 실행.
