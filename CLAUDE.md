...
1. **게임 관리** — 게임 추가(이름/상태 필수, 로고 선택, 게임별 문의 카테고리 커스터마이징), 게임 목록에서 선택. 새 게임에는 `lib/categories.ts`의 기본 종류·유형(계정/보안, 게임 이용, 결제/환불의 10개 유형)과 `lib/default-templates.ts`의 템플릿이 복사된다: 유형별 접수 안내(`DEFAULT_REPLY_TEMPLATES`, 자동 발송 켜짐)와 유형별 답변 본보기(`DEFAULT_RESOLUTION_TEMPLATES`, 자동 발송 꺼짐, AI 답변 추천의 말투·구조 본보기). 기존 게임에는 `scripts/seed-resolution-templates.ts`로 본보기를 채운다(같은 제목은 건너뜀, 2026-09-14 두 게임에 실행함). 템플릿 복사 실패는 경고만 낸다. 모든 게임 유형은 접수 폼에서 **스토어 종류**를 필수로 받는다(`inquiry_types.collects_store`, `inquiries.store`에 `google_play`/`app_store`/`onestore`/`other` 키 저장; 서비스 문의는 받지 않음. 마이그레이션은 실서비스 폼이 있는 theplayplus-web 저장소의 `0009`(열·결제/환불)·`0010`(모든 게임 유형)이 원본이고 여기 `0022`·`0023`은 사본; 2026-09-14 기준 아직 실행 전). 관리자에서는 `lib/format.ts`의 `STORE_LABELS`로 한국어 표기해 상세 패널 접수 정보와 대화 첫 말풍선에 "스토어" 행으로 보인다. 게임 생성 시 **운영 현황 구글 문서**(`lib/ops-doc.ts`, 제목 `{게임명} 운영 현황 (AI 답변 근거)`, 8개 섹션 뼈대)도 자동으로 만들어 문서 자료(`assistant_sources`)로 연결한다. 서비스 계정은 Drive 저장 용량이 0이라 문서를 소유할 수 없으므로(2026-09-14 실측) Gmail과 같은 OAuth 클라이언트로 발신 계정에 `drive.file` 스코프 토큰을 받아(`GOOGLE_DOCS_REFRESH_TOKEN`, `node scripts/get-gmail-refresh-token.js --docs`) 그 계정 소유로 만들고, 읽기용 서비스 계정·만든 관리자·`OPS_DOC_EDITORS`(쉼표 구분)에 편집자로 공유한다. `OPS_DOC_FOLDER_ID`를 주면 그 폴더 아래에 만든다. 토큰이 없으면 `ops_doc_not_configured`, 실패하면 `ops_doc_failed` 경고만 내고 게임은 만든다
...
5. **대화·이동** — 대화 열은 문의 본문(첨부 포함)·보낸 답변·사용자 회신·내부 메모를 시간순 한 줄기로 보여주고(`lib/timeline.ts`), 같은 게임·같은 `game_account`의 다른 문의도 접수 순으로 구분선을 두고 이어 보여준다(선택한 문의로 자동 스크롤). 작성란은 채팅 입력 상자 하나로 답변/메모 모드를 오간다(기능 3). 헤더의 이전/다음은 목록과 같은 조건·정렬 안에서 움직인다.
6. **새 문의 Slack 알림** — Supabase Database Webhook(`inquiries` INSERT)이 `/api/notify/inquiry`를 호출하면 게임명·유형·제목·상세 링크를 Slack Incoming Webhook으로 보낸다(`lib/slack.ts`). 호출자는 `x-webhook-secret` 헤더가 `INQUIRY_WEBHOOK_SECRET`과 일치해야 하고, `SLACK_WEBHOOK_URL`이 비어 있으면 조용히 건너뛴다. 알림은 **긴급(urgent) 게임 문의와 모든 서비스 문의**만 보낸다 — 우선순위는 before insert 트리거가 이미 정해 두므로 웹훅 payload의 `record.priority`를 그대로 보고 거르고(게임 문의가 긴급이 아니면 `{notified:false, skipped:"not_urgent"}`), 서비스 문의는 유형별 기본 우선순위 규칙이 없어 항상 `normal`이라 우선순위를 보지 않는다. 기준을 바꾸려면 이 라우트의 조건만 고치면 된다. 메시지에는 문의 본문도 인용문으로 함께 싣고(600자까지, 넘으면 `…`), 사용자가 쓴 글의 `& < >`는 escape한다. 색은 Slack attachment의 왼쪽 세로줄로 넣는다 — 긴급은 빨강 `#E01E5A`에 말머리가 "🚨 [게임명] 긴급 문의", 그 외는 회색 `#8D9298`에 "새 문의"다(색이 안 보이는 푸시 미리보기 때문에 말머리로도 구분). 서비스 문의는 게임명 자리에 "서비스 문의"가 들어가고 링크는 `/service/inquiries/{id}`다.
...
10. **운영 시트 어시스턴트** — **채팅은 기본 꺼짐**(2026-09-08, 답변 품질 미달). `lib/assistant-flags.ts`의 `ASSISTANT_CHAT_ENABLED=1`을 줘야 켜진다. 꺼져 있으면 문의함 보기 열의 링크가 "운영 자료 ↗"로 바뀌고 `/games/{gameId}/assistant`는 자료 목록·추가·연결 해제만 있는 `SourcesShell`을 그리며, 채팅 API(`/api/assistant/*`)는 404 `assistant_chat_disabled`를 돌려준다(자료 API `/api/games/{id}/sources`는 그대로). AI 답변 추천은 연결된 자료를 계속 읽는다. 켜져 있을 때: 문의함 보기 열의 "운영 어시스턴트 ↗"가 새 탭으로 `/games/{gameId}/assistant`를 연다(`app/(assistant)/`, 레일 없음). 게임에 연결한 구글 스프레드시트·구글 문서(`assistant_sources`, 사이드바의 "+ 자료 추가"에 URL 입력, 여러 개 가능, 마이그레이션 0020)를 서비스 계정(`GOOGLE_SERVICE_ACCOUNT_JSON`, 각 자료를 그 계정에 편집자로 공유)으로 매번 통째로 읽어 `# 시트: 제목`/`# 문서: 제목` 구간의 텍스트로 만들고 OpenAI(`OPENAI_API_KEY`, `OPENAI_MODEL` 기본 `gpt-5-mini`)에 시스템 프롬프트로 싣는다(`lib/assistant-sources.ts`, `lib/sheets.ts`, `lib/docs.ts`, `lib/assistant.ts`). 임베딩 검색은 쓰지 않고 자료 합계 30만 자까지다. 자료 제목은 등록 시 구글에서 읽어 저장하며, 공유가 안 돼 있으면 등록이 거절된다. "52009 VIP4로 올려줘" 같은 수정 요청은 모델이 `propose_update`/`propose_append` 도구(`spreadsheet` 인자로 시트 제목 지정)로 제안만 만들고, 서버가 자료·탭·열·행을 검증해 `sourceId`를 붙여 `assistant_messages`에 `pending`으로 저장하며, 관리자가 카드의 [적용]을 눌러야 그 시트를 다시 읽어 충돌을 확인한 뒤 Sheets API로 쓴다(마이그레이션 0018). 문서는 읽기 전용이고, 시트는 첫 줄이 열 이름인 탭만 수정할 수 있다. 대화는 게임별로 저장되고 ChatGPT식 사이드바에서 고른다(`?c=`). 입력창의 📎로 파일(txt/md/csv/tsv/json/xlsx, 파일당 4MB·메시지당 합계 4MB(Vercel 요청 본문 상한 4.5MB 아래)·5개·대화당 텍스트 200,000자)을 메시지에 붙일 수 있다: 서버가 텍스트만 뽑아 `assistant_messages.attachments` jsonb에 남기고(원본 미보관, 마이그레이션 0019, `lib/attachments.ts`; xlsx는 시트와 같은 표 형식으로 변환, utf-8이 아니면 EUC-KR로 해석) 그 대화의 모든 첨부를 프롬프트의 `# 첨부 파일` 섹션으로 싣는다. 파일은 읽기만 하고 수정 제안은 시트에만 한다. 설계는 `docs/superpowers/specs/2026-09-04-sheet-assistant-design.md`와 `docs/superpowers/specs/2026-09-07-assistant-sources-design.md`.
11. **AI 답변 추천** — 대화 열의 "AI 답변 추천"이 `POST /api/inquiries/{id}/suggest`로 답변 초안을 스트리밍한다(NDJSON, `lib/suggest.ts`). 모델은 어시스턴트와 같은 OpenAI(`OPENAI_API_KEY`, `OPENAI_MODEL` 기본 `gpt-5-mini`, `reasoning_effort: low` — minimal은 초안과 결론이 다른 템플릿이 오면 템플릿을 따랐다, 2026-09-14 실측). system 프롬프트에 **답변 구조**(사과·감사 → 배경 → 처리 방향·다음 단계 → 사용자가 할 일 → 개선 다짐, 단락마다 빈 줄, 이름표 없이)가 있고 호칭·첫 인사는 메일 템플릿이 붙이므로 쓰지 않게 한다. 초안은 완성 문장뿐 아니라 "~라고 말해줘" 같은 지시문일 수 있으며 그 지시를 받는 쪽은 모델이다(초안을 사용자의 말로 읽지 않게 규칙). 템플릿 중 자동 발송이 켜진 것은 "(접수 시 자동 발송된 안내)"로 표시해 답변 본보기에서 뺀다. 근거 다섯 가지를 프롬프트에 넣는다: (0) **작성 중인 초안** — 작성란에 적혀 있는 글을 버튼이 요청 본문 `draft`로 보내고(최대 5,000자) 초안이 있으면 user 메시지가 "## 이번 작업: 초안 다듬기"로 시작해 초안을 먼저 보여주고 나머지는 "참고 정보"로 뒤에 붙이며 끝에 한 번 더 상기시킨다(초안을 자료 뒤에 한 구간으로 덧붙이면 gpt-5-mini가 템플릿·이력으로 새 답변을 쓰고 초안의 결정을 뒤집었다, 2026-09-08 실측). 제안은 그 초안을 바로 덮어쓰고 "되돌리기"로 복구한다, (0-1) **이 문의의 대화 이력** — `inquiry_messages`(보낸 답변·자동 답변·사용자 회신)와 내부 메모를 시간순으로 `[보낸 답변 · 2026-09-02 10:00]` 식 머리말과 함께 싣고(항목당 2,000자, 최근 20건) 후속 답변으로 이어 쓰게 한다(내부 메모는 사실 확인용, 문장을 옮기지 않게 규칙), (1) 유형 템플릿, (2) **유사 과거 답변** — 문의 제목+본문을 `text-embedding-3-small`로 임베딩해 `inquiries.embedding`(pgvector, 마이그레이션 0021)에 저장하고 `match_answered_inquiries` RPC로 같은 스코프(게임 하나 또는 서비스 문의)에서 유사도 0.35 이상 상위 5건의 "문의 요약 + 첫 수동 답변"을 가져오며 2건 미만이면 같은 유형 최근 3건으로 보충(`lib/embeddings.ts`, `lib/replies.ts`), (3) **운영 자료** — 게임 문의면 어시스턴트에 연결된 시트·문서 전부를 `loadSources`로 읽어 system 프롬프트 뒤에 싣는다(서비스 문의는 없음). 임베딩은 추천 클릭 시와 수동 답변 발송 시 채우고, 기존 문의는 `scripts/backfill-inquiry-embeddings.js`로 한 번 채운다. 모델은 본문 뒤에 `=== 근거 ===` 구분선과 참고 자료 목록을 쓴다(`lib/suggest-evidence.ts`). 화면에는 미리보기 카드가 없다: 제안이 입력 상자에 바로 써지고(쓰는 동안 상자가 주황 점선, 입력 잠김, "중단" 가능), 끝나면 상자 아래 한 줄에 캡션·경고·참고한 자료(종류별 아이콘)·"되돌리기"(누르기 전 글로 복구, 편집하면 사라짐)가 나온다(`components/inquiries/useSuggestion.ts`). 작성란에 적혀 있던 초안은 요청 본문 `draft`로 보내며 모델은 그 초안의 내용·결정을 유지해 다듬는다. 자료 읽기 실패·임베딩 실패는 `warning` 이벤트로 본문보다 먼저 내려가고 추천은 계속 만든다. 막히는 건 API 키 없음뿐이다. 설계는 `docs/superpowers/specs/2026-09-07-openai-suggest-design.md`.
...
1. `OPENAI_API_KEY`가 설정돼 있어야 한다(어시스턴트와 공유). Gemini 키는 더 이상 쓰지 않는다. 배경 설명에 쓸 운영 사실(현재 서비스 상태, 알려진 이슈, 점검 일정, 정책)은 게임별 "운영 현황" 구글 문서에 적어 운영 자료로 연결한다 — 두 게임에는 2026-09-14에 뼈대 문서를 만들어 연결해 두었고 내용은 비어 있다. 문서에 "(예: …)" 같은 가짜 사실을 적으면 모델이 그대로 답변에 옮기므로 확실한 사실만 적는다. Google Cloud 프로젝트에 Google Docs API가 켜져 있어야 한다(2026-09-14에 켬, 그 전에는 문서 자료가 모두 `source_forbidden`이었다)
...
5. 새 게임에 운영 현황 문서가 자동으로 만들어지게 하려면 `node scripts/get-gmail-refresh-token.js --docs`를 문서를 소유할 계정(help@)으로 한 번 실행해 `GOOGLE_DOCS_REFRESH_TOKEN`을 넣고 재배포한다. Google Cloud 프로젝트에 Google Drive API도 켜져 있어야 한다(2026-09-14에 켬). 게임을 하나 만들어 운영 자료 목록에 문서가 붙고 관리자 계정으로 열리는지 확인한다
