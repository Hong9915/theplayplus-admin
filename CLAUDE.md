...
3. Supabase SQL Editor에서 `0017_assistant.sql`, `0018_assistant_attachments.sql`, `0019_assistant_sources.sql`을 순서대로 실행. 0017은 anon의 games 조회를 열 단위로 제한한다 — 접수 폼은 id/name/logo_path만 읽는다. 0019는 기존 `games.sheet_id`를 자료 표로 옮기고 열을 지운다
