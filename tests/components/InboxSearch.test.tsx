...
describe("InboxSearch", () => {
  ...
  it("shows the current term", () => {
    render(<InboxSearch scope={gameScope("g1")} query={{ ...DEFAULT_QUERY, q: "환불" }} selectedId={null} />);
    expect(screen.getByLabelText("검색")).toHaveValue("환불");
  });

  it("debounces typing into the URL and resets the page", async () => {
    render(<InboxSearch scope={gameScope("g1")} query={{ ...DEFAULT_QUERY, page: 3 }} selectedId={null} />);
  ...
  it("keeps the selected inquiry in the URL", async () => {
    render(<InboxSearch scope={gameScope("g1")} query={DEFAULT_QUERY} selectedId="i1" />);
