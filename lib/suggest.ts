...
export interface SuggestInput {
  ...
  /** autoSend가 켜진 템플릿은 접수 시 자동 발송된 안내라 답변 본보기가 아니다. */
  templates: Array<{ title: string; content: string; autoSend?: boolean }>;
...
const SYSTEM_RULES = [
  ...
  "- 호칭과 첫 인사('OOO님, 안녕하세요. 답변드립니다')는 발송 시스템이 본문 앞에 붙입니다.",
  "  본문은 인사 없이 바로 시작하세요.",
  "- 참고 템플릿이 주어지면 그 말투와 구조를 따르세요. 템플릿의 제목은 참고용",
  "  이름일 뿐이니 답변 본문에 옮겨 적지 마세요.",
  "- '(접수 시 자동 발송된 안내)'가 붙은 템플릿은 접수 직후 사용자에게 이미 나간 접수 확인",
  "  문구입니다. 말투만 참고하고, 그 구조(접수되었다는 안내, 정보 요청 목록)를 답변으로",
  "  되풀이하지 마세요. 그 표시가 없는 템플릿이 답변 본보기입니다.",
  ...
  "- 초안은 완성된 문장이 아니라 '~라고 말해줘', '~하라고 안내해', 핵심어 나열 같은 관리자의",
  "  지시일 수 있습니다. 그 지시를 받는 사람은 당신입니다. 지시대로 사용자에게 전할 내용을 쓰고,",
  "  '말해줘'·'안내해' 같은 지시 표현은 메일에 남기지 마세요. 초안을 사용자가 한 말로 읽지 마세요.",
  ...
  "",
  "답변 구조:",
  "- 단락마다 빈 줄을 두고, 한 단락은 한두 문장으로 짧게 쓰세요.",
  "- 순서는 다음과 같습니다. 해당 사항이 없는 단락은 빼고, 자리를 채우려고 뻔한 문장을 넣지 마세요.",
  "  1) 불편·지연에 대한 사과 또는 문의해 주신 데 대한 감사 한 문장.",
  "  2) 배경·현재 상황 설명. 참고 자료·대화 이력·초안에 있는 사실로만 쓰세요.",
  "  3) 이 건을 어떻게 처리하는지와 다음 단계. '담당 부서에 전달하겠습니다', '원인이 확인되면",
  "     수정하겠습니다', '확인 후 본 메일로 안내드리겠습니다'처럼 무엇을 언제 하는지 구체적으로 쓰세요.",
  "     이런 절차 약속은 써도 되지만, 결과(지급·환불·수정 완료)와 기한은 확인된 것만 쓰세요.",
  "  4) 사용자가 해야 할 일이 있으면 무엇을 어떻게 보내면 되는지 분명히 쓰세요.",
  "  5) 불편에 대한 사과와 개선 다짐, 맺음 인사.",
  "- 위 번호와 단락 이름은 순서 안내일 뿐입니다. '현재 상황:', '다음 단계:' 같은 이름표나",
  "  번호를 본문에 쓰지 말고 문장으로만 이어 쓰세요.",
  "",
  "출력 형식:",
...
export function buildSuggestPrompt(input: SuggestInput): { system: string; userMessage: string } {
  ...
  if (input.templates.length > 0) {
    ...
    input.templates.forEach((template, index) => {
      ...
      // 본문과 시각적으로 분리한다.
      const label = template.autoSend ? `${template.title} (접수 시 자동 발송된 안내)` : template.title;
      lines.push("", `${index + 1}) 템플릿 이름: ${label}`, "본문:", template.content);
    });
  }
  ...
  const userMessage = [
    ...
    "",
    "---",
    "다시 한 번: 위 '초안'을 답변 구조에 맞춰 다듬은 메일 본문만 출력하세요. 초안에 없는 사실·결정을 덧붙이지 마세요.",
  ].join("\n");
...
const DRAFT_TASK = [
  ...
  "- 초안이 '~라고 말해줘', '~하라고 안내해', 핵심어 나열 같은 지시문이면 그 지시를 받는 사람은 당신입니다.",
  "  지시한 내용을 사용자에게 직접 말하는 문장으로 쓰고, 초안을 사용자가 한 말로 읽어 되풀이하지 마세요.",
  "  '~라고 안내드립니다', '~라고 말씀드립니다'처럼 지시를 옮겨 적는 문장도 쓰지 마세요.",
  "  예: 초안 '스토어로 문의해서 환불 됐다고 말해줘' → '스토어를 통해 문의하신 건은 환불 처리가 완료되었습니다.'",
  "- 초안에 없는 새 사실·결정·요청(추가 정보 요청, 보상 조건, 처리 기한, 환불 가능 여부 등)을 덧붙이지 마세요.",
  "- 사과·감사, 초안의 결정을 실행하는 절차('담당 부서에 전달하겠습니다' 등), 개선 다짐, 맺음 인사는",
  "  답변 구조에 맞춰 채우세요. 초안이 한 줄이어도 서너 단락의 완성된 메일이 되어야 합니다.",
  "- 아래 참고 정보(문의 내용·대화 이력·템플릿·과거 답변)는 말투와 사실 확인에만 쓰세요.",
  "  템플릿·과거 답변의 결론이 초안과 다르면 초안을 따르세요. 초안이 '환불 됐다'면 환불이 완료된 것이고,",
  "  템플릿이 '스토어에 신청하라'고 해도 그 절차를 안내하지 마세요.",
];
...
export async function* streamSuggestion(input: SuggestInput): AsyncGenerator<SuggestEvent> {
  ...
  try {
    ...
    const stream = await client.chat.completions.create({
      ...
      stream: true,
      // minimal은 초안("환불 됐다")과 결론이 다른 템플릿("스토어에 신청하라")이
      // 함께 오면 규칙을 적어 줘도 템플릿을 따랐다(2026-09-14 실측). low부터
      // 초안을 지킨다. gpt-5 계열은 temperature를 기본값 외로 주면 거절하므로
      // 넣지 않는다.
