...
import { deleteTemplate, setTemplateAutoSend } from "@/lib/templates";

// 삭제·수정에 필요한 것은 템플릿 id뿐이다. /api/games/[gameId]/templates/[id]로
// 중첩하면 gameId가 검증에도 쓰이지 않으면서 URL만 길어진다.

const patchSchema = z.object({ autoSend: z.boolean() });

/** 자동 발송 켜기/끄기. 켜면 같은 게임·유형의 다른 템플릿은 꺼진다. */
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const updated = await setTemplateAutoSend(supabase, params.id, parsed.data.autoSend);
